...
#define PTE_Rw      0x30    // executable user page(subpage disable)

// 1st-level or large (1MB) page directory (always maps 1MB memory) -> It doesn't mean pgdir is 1MB. 
// It just maps 1 MB of physical memory to virtual PTE's.
...
#define PTE_SZ      (1 << PTE_SHIFT) // Page size (not the size of page table entry ptr)
...
#define MAXVA   0x7FFFFFFF



// Safety ranges for selecting candidate VA
#define USER_SAFE_LO   (3 * PTE_SZ)
#define USER_SAFE_HI   (UADDR_SZ - 3 * PTE_SZ)


// Extract frame (physical base) from a pte value
#define PTE_FRAME(pte)    ((pte) & ~(PTE_SZ - 1))

// Basic predicates
// true if PTE holds a small-page / valid second-level mapping
#define PTE_IS_VALID(pte) (((pte) & PE_TYPES) == PTE_TYPE)

// user-access check: returns non-zero if PTE grants user access (read or read/write)
#define PTE_IS_USER(pte)  (PTE_AP(pte) == AP_KU || PTE_AP(pte) == AP_KUR)
