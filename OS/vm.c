...
pte_t* walkpgdir (pde_t *pgdir, const void *va, int alloc) //pte_t and pde_t are uint32
{
    // Returns the pointer to the page table entry
    // Logic:
    // if the page is not there -> it allocates (if alloc = 1) a new page
    pde_t *pde;
    pte_t *pgtab;

    // pgdir points to the page directory, get the page direcotry entry (pde)
    pde = &pgdir[PDE_IDX(va)]; // pde = page directory entry

    if (*pde & PE_TYPES) { //Last 2 bits are valid bit and (read-write bit)
        // get the pointer to the page table 
        pgtab = (pte_t*) p2v(PT_ADDR(*pde));

    } else {
        // if reading(alloc = 0) -> and if invalid page -> return 0 (error)
        // elif writing(alloc = 1) -> and allocation fails -> return 0 (error)
        ...
        *pde = v2p(pgtab) | UPDE_TYPE; // Making the last bit 1
        // turning on the 0th bit =  makes it valid
    }
...
{   
    // cprintf("pde_t *pgdir: %x, void *va %x, uint size %x, uint pa %x, int ap %x\n",*pgdir,(uint)va,size,pa,ap);
    char *a, *last;
    pte_t *pte;
...
int loaduvm (pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
    ...
    for (i = 0; i < sz; i += PTE_SZ) {
        ...
        //     int newsz = addr + PTE_SZ;
        //     int allocuvm (pgdir, addr, newsz);
...
// Try to find a present user PTE inside [USER_SAFE_LO, limit).
// Returns a victim VA (aligned to PTE_SZ) or 0 if none found.
static uint find_evict_va(pde_t *pgdir, int max_tries) {
    uint limit = USER_SAFE_HI;

    int tries = 0;
    while (tries < max_tries) {
        tries++;
        uint32 r = rand();
        uint range = (limit - USER_SAFE_LO);
        if (range == 0) return 0;

        uint32 va = (r % range) + USER_SAFE_LO;
        va = align_up(va, PTE_SZ);

        pte_t *pte = walkpgdir(pgdir, (char*)va, 0);
        if (!pte) continue;

        uint pteval = *pte;
        if (!PTE_IS_VALID(pteval)) continue;
        if (!PTE_IS_USER(pteval)) continue;
...
// Evil evict that chooses a victim inside the process allocated limit
// If limit==0, it will use USER_SAFE_HI as the upper bound.
void evil_evict_limit(pde_t *pgdir, uint limit) {
    n_pgevict++;

    if (limit == 0) limit = USER_SAFE_HI;
    uint va = find_evict_va(pgdir, 4096); // try up to 128 random probes
    if (va == 0) {
        cprintf("evict: no evictable page found (limit=%x)\n", limit);
        return;
    }

    pte_t *pte = walkpgdir(pgdir, (char*)va, 0);
    if (!pte) {
        cprintf("evict: walkpgdir returned NULL for va %x unexpectedly\n", va);
        return;
    }

    uint pteval = *pte;
    if (!PTE_IS_VALID(pteval) || !PTE_IS_USER(pteval)) {
        cprintf("evict: candidate va %x lost validity; skipping\n", va);
        return;
    }

    uint pa = PTE_FRAME(pteval);
    if (pa == 0) {
        cprintf("evict: candidate va %x has pa==0; skipping\n", va);
        return;
    }
    *pte = 0;
    free_page(p2v(pa));

    cprintf("evict: freed VA %x PA %x. total evicted %d\n", va, pa, n_pgevict);
}

// Replace allocuvm with this safer version
int allocuvm (pde_t *pgdir, uint oldsz, uint newsz)
{
    char *mem;
    uint a;
    ...
    for (; a < newsz; a += PTE_SZ) {
        ...
        if (mem == 0) {
            // out of memory: attempt a bounded number of evictions and retries
            cprintf("allocuvm: out of memory, attempting evictions");

            int tries = 0;
            const int MAX_EVICT_ROUNDS = 1;   // number of times to attempt an eviction cycle
            while (mem == 0 && tries < MAX_EVICT_ROUNDS) {
                // ask evil_evict to pick victims only inside [0, oldsz)
                // if oldsz==0, supply a safe upper bound
                evil_evict_limit(pgdir, oldsz);
                mem = alloc_page();
                tries++;
            }

            if (mem == 0) {
                cprintf("allocuvm: out of memory after evictions (tried=%d)\n", tries);
                return 0;
            }
        }
...
void kpt_print(void)
{
    cprintf("Kernel Page Directory Entries (from KERNBASE):\n");
    pde_t *pgdir = (pde_t*)P2V(&_kernel_pgtbl);
    int count = 0;

    // KERNBASE's index in the page directory is 0x80000000 >> 20 = 2048
    for (int i = PDE_IDX(KERNBASE); i < 4096 && count < 15; i++) {
        pde_t pde = pgdir[i];
        // Check if it's a valid 1MB section entry
        if ((pde & 0x3) == 0x2) {
            uint va = i << 20;
            uint pa = pde & 0xFFF00000;
            cprintf("  PDE[%d] VA 0x%x -> PA 0x%x (1MB Section)\n", i, va, pa);
            count++;
        }
    }
}